...
type PreknowProductReviewsProps = {
  className?: any;
  productId: string;
  productType?: string;
};

const PreknowProductReviews: React.FC<PreknowProductReviewsProps> = ({
  productId,
  productType,
}) => {
  ...
  return (
    <div className="mx-auto max-w-screen-xl  rounded-md bg-light px-5 pb-14 xl:px-0">
      <div
        className={cn(
          'border-t border-b border-border-200 border-opacity-70 px-5'
        )}
      >
        <div
          className={cn(
        ...
        >
          <h2 className="mt-3 text-lg font-semibold tracking-tight text-heading sm:mt-0">
            Đánh giá - Nhận xét từ khách hàng ({paginatorInfo?.total ?? 0})
          </h2>
...
export default PreknowProductReviews;
